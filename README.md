...
- [ ] **(重要,未爆彈)** 題目 ID 穩定化。目前 `id = s{gid}-{全域流水號}`,
  流水號是**三個工作表共用**的,所以只要在前兩個分頁增刪任何一列,
  後面所有分頁的 ID 會整批位移(實測:在第一個分頁加 1 題 → 67% 的
  ID 改變),使用者的作答紀錄會整批掛到別的題目上。
  目前沒事只因為題庫還沒被改過。**在追加題目之前必須先修**。
  修法:改成 `s{gid}-r{該分頁的實際列號}`,並在 build 時同時輸出舊
  ID 供前端換算,既有紀錄就不會斷。
- [ ] 未作答 / 被捨棄的題目目前不計入「做過」(已討論後決定維持),
  若之後覺得重複感仍在,這是下一個可調的旋鈕。
